...
class Auth extends Common
{
    ...
    public function adminList(){
        if(Request::isAjax()){
            ...
            $list=Db::table(config('database.prefix').'admin')->alias('a')
                ...
                ->join(config('database.prefix').'organization_group og','a.ogroup_id = og.id','left')
                ->field('a.*,ag.title,og.group_name')
                ->where($map)
                ->select();
    ...
    public function adminAdd(){
        if(Request::isAjax()){
            ...
            $ogroup = Ogroup::where('is_use=1')->all();
    ...
    public function adminEdit(){
        if(request()->isPost()){
            ...
            if( $data['admin_id'] == session('aid')){
                ...
                $avatar = $data['avatar']==''?'/muke/public/static/admin/images/0.jpg':$data['avatar'];
    ...
    //组织组管理
    public function ogroupList(){
        if(request()->isPost()){
            $list = Ogroup::where('is_use=1')->all();
            return $result = ['code'=>0,'msg'=>'获取成功!','data'=>$list,'rel'=>1];
        }
        return view('ogroupList');
    }
    //删除组织分组
    public function ogroupDel(){
        Ogroup::where('id','=',input('id'))->update(['is_use'=>'0']);
        return $result = ['code'=>1,'msg'=>'删除成功!'];
    }
    //添加组织分组
    public function ogroupAdd(){
        if(request()->isPost()){
            $data=input('post.');
            $data['addtime']=time();
            Ogroup::create($data);
            $result['msg'] = '组织添加成功!';
            $result['url'] = url('ogroupList');
            $result['code'] = 1;
            return $result;
        }else{
            $this->assign('title','添加组织分组');
            $this->assign('info','null');
            return $this->fetch('ogroupForm');
        }
    }
    //修改组织分组
    public function ogroupEdit(){
        if(request()->isPost()) {
            $data=input('post.');
            $where['id'] = $data['id'];
            Ogroup::update($data,$where);
            $result = ['code'=>1,'msg'=>'组织分组修改成功!','url'=>url('ogroupList')];
            return $result;
        }else{
            $id = input('id');
            $info = Ogroup::get(['id'=>$id]);
            $this->assign('info', json_encode($info,true));
            $this->assign('title','编辑组织分组');
            return $this->fetch('ogroupForm');
        }
    }
    //分组配置课程规则
    public function ogroupAccess(){
        $nav = new Leftnav();
        $rule_all=db('video_type')->field('id,fid,type')->where('is_show=1')->order('sort asc')->select();
        $rule_group = db('organization_group')->where('id',input('id'))->value('course_power');
        $arr = $nav->auth($rule_all,$fid=0,$rule_group);
        $arr[] = array(
            "id"=>0,
            "fid"=>0,
            "type"=>"全部",
        ...
        $this->assign('data',json_encode($arr,true));
        return $this->fetch('ogroupAccess');
    }
    public function ogroupSetaccess(){
        $course_power = input('post.course_power');
        if(empty($course_power)){
            return array('msg'=>'请选择权限!','code'=>0);
        }
        $data = input('post.');
        $where['id'] = $data['id'];
        if(Ogroup::update($data,$where)){
            return array('msg'=>'权限配置成功!','url'=>url('ogroupList'),'code'=>1);
        }else{
            return array('msg'=>'保存错误','code'=>0);
        }
    }
