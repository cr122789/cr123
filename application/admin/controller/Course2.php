...
Class Course extends Common{
    ...
    public function add(){
    	...
    	if(request()->isPost()){
		   ...
		   //tag
            ...
            $res = db('video_video')->insert($list);
            if($res){
    ...
    public function edit($id=''){
        if(request()->isPost()){
		   ...
		   if($course_video['video']!=$post['video']){
            ...
            $info=db('video_video')->where('id',$id)->find();
            if($info['tag_id']){
    ...
    public function questionList($id=''){
        if(request()->isPost()){
            ...
            $list=db('video_questions')->where('video_id',$video_id)->where('is_show=1')->select();
            foreach($list as &$v){
    ...
    public function questionEdit($question_id = ''){
        if(request()->isPost()){
            ...
            $this->assign('video_id',$info['video_id']);
            $this->assign('title','创建问题');
            return $this->fetch('questionForm');
        }
    }
    public function questionDel(){
        $id = input('post.id');
        // db('video_questions')->where(array('id'=>$id))->delete();
        // db('video_answers')->where(array('question_id'=>$id))->delete();
        db('video_questions')->where(array('id'=>$id))->update(['is_show'=>'0']);
        db('video_answers')->where(array('question_id'=>$id))->update(['is_show'=>'0']);
